...
export async function render(container, { user }) {
  ...
  _container.querySelector('#contacts-import-input').addEventListener('change', async (e) => {
    ...
    try {
      ...
      if (!contacts.length) { window.planium?.showToast(t('contacts.vcardNoName'), 'warning'); return; }
      ...
      window.planium?.showToast(t('contacts.importError', { error: err.message }), 'danger');
    }
  });
}
...
function renderSelectBar() {
  ...
  moveDropdown.querySelectorAll('.csb-move-option').forEach((opt) => {
    opt.addEventListener('click', async () => {
      ...
      window.planium?.showToast(`${ids.length} contact(s) moved to ${CATEGORY_LABELS()[cat] || cat}`, 'success');
      exitSelectMode();
    });
  });
  ...
  openSharedModal({
    ...
    onSave(panel) {
      ...
      panel.querySelector('#cm-save').addEventListener('click', async () => {
        ...
        if (!name) { window.planium?.showToast(t('common.nameRequired'), 'error'); return; }
        ...
        try {
          ...
          closeModal();
          renderList();
          window.planium?.showToast(mode === 'create' ? t('contacts.savedToast') : t('contacts.updatedToast'), 'success');
        } catch (err) {
...
async function deleteContact(id) {
  try {
    ...
    renderList();
    vibrate([30, 50, 30]);
    window.planium?.showToast(t('contacts.deletedToast'), 'success');
  } catch (err) {
    window.planium?.showToast(err.data?.error ?? t('common.unknownError'), 'error');
  }
}
