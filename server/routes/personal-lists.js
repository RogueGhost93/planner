...
router.post('/', (req, res) => {
  try {
    ...
    const quick_done    = req.body.quick_done    !== undefined ? (req.body.quick_done    ? 1 : 0) : 0;
    ...
    const id = db.transaction(() => {
      ...
      const result = db.get().prepare(`
        INSERT INTO task_lists (owner_id, name, color, sort_order, show_priority, quick_done)
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(req.session.userId, vName.value, vColor.value || '#2563EB', maxOrder + 1, show_priority, quick_done);
...
router.put('/:id', (req, res) => {
  try {
    ...
    const newQuickDone    = req.body.quick_done    !== undefined ? (req.body.quick_done    ? 1 : 0) : list.quick_done;

    db.get().prepare('UPDATE task_lists SET name = ?, color = ?, show_priority = ?, quick_done = ? WHERE id = ?')
      .run(newName, newColor, newShowPriority, newQuickDone, req.params.id);
